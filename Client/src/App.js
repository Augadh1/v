...
class App extends React.Component {
    ...
    this.state = {
      db: {
        ...
        pastorder: [
          {
            date: "12-08-2019",
            item: { name: "Coffiee", quantity: 5, price: 50 },
            totalprice: 150,
          },
          {
            date: "12-08-2019",
        ...
        monthsales: 20000,
        daysales: 1000,
        monthsalescoffee: 8000,
        monthsalestea: 8000,
        monthsaleswater: 4000,
        daysalescoffee: 400,
        daysalestea: 500,
        daysaleswater: 300,
      },
  ...
  checkLogin() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      ...
      axios.put("http://localhost:8080/user", { uid: user.uid }).then((res) => {
        ...
        axios
          .put("http://localhost:8080/pastorder", { uid: user.uid })
          .then((res) => {
  ...
  userLogIn = () => {
    ...
    firebase
      .auth()
      .signInWithEmailAndPassword(db.email, db.password)
      .then((result) => {
        var user = result.user;
        console.log("email signin", user.displayName, user.email, user.uid);
        axios
          .put("http://localhost:8080/user", { uid: user.uid })
          .then((res) => {
            console.log(res.data);
            db.user = res.data;
            db.user.password = "******";
  ...
  addUser = () => {
    ...
    if (db.passwordConfirmation !== db.password) {
      ...
      firebase
        .auth()
        .createUserWithEmailAndPassword(db.email, db.password)
        .then((result) => {
          var user = result.user;
          axios
  ...
  forgot = () => {
    firebase
      .auth()
      .sendPasswordResetEmail(this.state.db.email)
      .then(function (user) {
  ...
  submitfeeds = () => {
    ...
    axios.get("http://localhost:8080/feedback").then((res) => {
      const { db } = this.state;
      db.feedbacks = res.data;
      console.log(db);
      this.setState({ db });
  ...
  calcsumary = () => {
    ...
    axios.get("http://localhost:8080/sumary").then((res) => {
      const { db } = this.state;
    ...
    axios.get("http://localhost:8080/sumarydays").then((res) => {
      const { db } = this.state;
  ...
  payment = () => {
    ...
    axios
      .post("http://localhost:8080/payment", {
        data: "tok_1FlpT8BSB3liGtsaHXMbWYnO",
      ...
      .then((res) => {
        ...
        this.setState({ db });
        axios
          .post("http://localhost:8080/Order", { pastorder: db.orderItem })
          .then((res) => {
            console.log("hiii hello how are you");
            console.log(res.data);
          });
        ...
        <Switch>
          ...
          <Route
            ...
            render={(props) => (
              <Dashboard
                {...props}
                submitfeeds={this.submitfeeds}
                handlechange={this.handleChange}
                handleItemClick={this.handleItemClick}
                db={this.state.db}
                placeorder={this.placeorder}
                handlequantity={this.handlequantity}
                profileedit={this.edit_profile}
              />
            )}
          />
          <Route
          ...
          <Route
            ...
            render={(props) => <PlacedOrder {...props} db={this.state.db} />}
          />
          <Route
            exact
            path="/payment"
            ...
            <Route
              ...
              render={(props) => (
                <DashboardAdmin
                  ...
                  db={this.state.db}
                  placeorder={this.placeorder}
                  usersignup={this.addAdmin}
                  userdashboard={this.userdashboard}
                  profileedit={this.edit_profile}
                  handlechange={this.handleChange}
                  calcsumary={this.calcsumary}
                />
              )}
            />
          )}
